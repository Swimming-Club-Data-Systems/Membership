...
$pagetitle = htmlspecialchars((string) $row['ExtraName']) . " - Extras";
...
<div class="container-xl">
  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page"><?= htmlspecialchars((string) $row['ExtraName']) ?></li>
    </ol>
  </nav>

  <div class="row align-items-center mb-3">
    <div class="col-md-8">
      <h1><?= htmlspecialchars((string) $row['ExtraName']) ?> <small>&pound;<?= htmlspecialchars(number_format($row['ExtraFee'], 2)) ?>/month (<?php if ($row['Type'] == 'Payment') { ?>payment<?php } else { ?>credit/refund<?php } ?>)</small></h1>
    </div>
    <div class="col text-sm-end">
      <a href="<?= htmlspecialchars((string) autoUrl("payments/extrafees/$id/edit")) ?>" class="btn btn-dark-l btn-outline-light-d">Edit</a>
      <a href="<?= htmlspecialchars((string) autoUrl("payments/extrafees/$id/delete")) ?>" class="btn btn-danger">Delete</a>
    </div>
  </div>
  <div class="row">
    <div class="col order-md-1 mb-3">
      <div class="card">
        ...
        <form class="card-body">
          <div class="mb-3">
            ...
            <select class="form-select" id="squadSelect" name="squadSelect">
              ...
              <?php while ($squadsRow = $squads->fetch(PDO::FETCH_ASSOC)) { ?>
                <option value="<?= htmlspecialchars((string) $squadsRow['SquadID']) ?>">
          ...
          <button disabled type="button" class="btn btn-success" id="addSwimmer" data-ajax-url="<?= htmlspecialchars((string) autoUrl("payments/extrafees/ajax/" . $id)) ?>">
            Add member to extra
          </button>
          <div id="status">
          </div>
