  ...
  <div class="container-xl">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page"><?= htmlspecialchars((string) $session->getUser()->getName()) ?></li>
      </ol>
    </nav>
    ...
    <div class="col-lg-8">
      ...
      <form method="post" class="needs-validation" id="form" novalidate>
        ...
        <div class="mb-3">
          ...
          <input type="date" class="form-control" id="start-date" name="start-date" value="<?= htmlspecialchars((string) $session->start->format('Y-m-d')) ?>" required aria-describedby="start-help" <?php if ($session->renewal) { ?>disabled<?php } ?>>
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control" id="welcome-text" name="welcome-text" rows="5" <?php if ($session->renewal) { ?>disabled<?php } ?>><?= htmlspecialchars((string) $session->welcomeText) ?></textarea>
        </div>
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="status" id="status" <?php if ($session->status == 'not_ready' || $session->renewal) { ?>disabled<?php } ?>>
            <?php foreach ($states as $key => $value) { ?>
              <option <?php if ($session->status == $key) { ?>selected<?php } ?> value="<?= htmlspecialchars((string) $key) ?>"><?= htmlspecialchars((string) $value) ?></option>
            <?php } ?>
          </select>
        </div>
        ...
        <div class="collapse <?php if ($session->dueDate) { ?>show<?php } ?>" id="due-date-box">
          <div class="mb-3">
            ...
            <input type="date" class="form-control" id="due-date" name="due-date" value="<?php if ($session->dueDate) { ?><?= htmlspecialchars((string) $session->dueDate->format('Y-m-d')) ?><?php } ?>" <?php if ($session->renewal) { ?>disabled<?php } ?>>
          </div>
        </div>
        ...
        <ul class="mb-3">
          <?php foreach ($session->members as $member) { ?>
            <li><?= htmlspecialchars((string) \SCDS\Formatting\Names::format($member->firstName, $member->lastName)) ?></li>
          <?php } ?>
        </ul>
        ...
        <?php if ($session->status != 'complete' && $session->batch) { ?>
          <p>
            <a href="<?= htmlspecialchars((string) autoUrl("memberships/batches/" . $session->batch . "/edit")) ?>" class="btn btn-success">Edit fees</a>
          </p>
