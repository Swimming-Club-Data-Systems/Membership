...
<div class="container-xl">
  <div class="row">
    <div class="col-md-8">
      ...
      <p class="lead">This software is licenced to <?= htmlspecialchars((string) app()->tenant->getKey('CLUB_NAME')) ?> by Swimming Club Data Systems.</p>
      ...
      <div class="card card-body mb-3">
        <h3 class="card-title">
          Contact <?= htmlspecialchars((string) app()->tenant->getName()) ?> in the first instance
        </h3>

        <dl class="row mb-0">
          <dt class="col-sm-3">Email</dt>
          <dd class="col-sm-9 text-truncate mb-0"><a href="mailto:<?= htmlspecialchars((string) app()->tenant->getKey('CLUB_EMAIL')) ?>"><?= htmlspecialchars((string) app()->tenant->getKey('CLUB_EMAIL')) ?></a></dd>
        </dl>
      </div>
      ...
      <div class="card card-body mb-3">
        <dl class="row mb-0">
          ...
          <dd class="col-sm-9 font-monospace text-truncate"><?= htmlspecialchars((string) $tenant->getUuid()) ?></dd>

          <dt class="col-sm-3">Tenant ID</dt>
          <dd class="col-sm-9 font-monospace"><?= htmlspecialchars((string) $tenant->getId()) ?></dd>

          <dt class="col-sm-3">Tenant Name</dt>
          <dd class="col-sm-9 font-monospace"><?= htmlspecialchars((string) $tenant->getName()) ?></dd>

          <?php if ($tenant->getCode()) { ?>
            <dt class="col-sm-3">Tenant Code</dt>
            <dd class="col-sm-9 font-monospace"><?= htmlspecialchars((string) $tenant->getCode()) ?></dd>
          <?php } ?>

          <?php if (isset(app()->user)) { ?>
            <dt class="col-sm-3">User</dt>
            <dd class="col-sm-9 font-monospace mb-0"><?= htmlspecialchars((string) app()->user->getId()) ?></dd>
          <?php } else { ?>
            <dt class="col-sm-3">User</dt>
      ...
      <p>This application has been integrated with GoCardless and their APIs to allow <?= htmlspecialchars((string) app()->tenant->getKey('CLUB_NAME')) ?> to bill members by Direct Debit. The GoCardless client library which is included in this software is copyright of GoCardless.</p>
