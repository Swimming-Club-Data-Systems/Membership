...
try {
  ...
  $email = mb_strtolower(trim((string) $_POST['new-cc']));
  ...
  $name = mb_ucfirst(trim((string) $_POST['new-cc-name']));
  ...
  $message = '
  <p>Hello ' . htmlspecialchars((string) $name) . ',</p>
  <p>' . htmlspecialchars($_SESSION['TENANT-' . app()->tenant->getId()]['Forename'] . ' ' . $_SESSION['TENANT-' . app()->tenant->getId()]['Surname']) . ' wishes for you to also get emails from ' . htmlspecialchars((string) app()->tenant->getKey('CLUB_NAME')) . '.</p>
  <p>Please follow the link below to verify your email address.</p>
  <p><strong><a href="' . autoUrl($link) . '">' . autoUrl($link) . '</a></strong></p>
  <p>This will confirm your email address and send carbon copies of emails from coaches and committee members at ' . htmlspecialchars((string) app()->tenant->getKey('CLUB_NAME')) . ' to this address.</p>
