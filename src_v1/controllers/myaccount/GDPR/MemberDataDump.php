...
header('Content-Disposition: attachment; filename=GDpe-Data-Download-' . str_replace(' ', '-', (string) $swimmer_name) . '.csv');

// create a file pointer connected to the output stream
$output = fopen('php://output', 'w');

fputcsv($output, [app()->tenant->getKey('CLUB_NAME') . ' Member Data Download']);
...
fputcsv($output, ['Data downloaded at ' . date("H:i, d/m/Y")]);
fputcsv($output, ['-----']);

fputcsv($output, ['Parent Account Details']);
fputcsv($output, ['Forename', 'Surname', 'Email Address', 'Phone', 'Email Allowed', 'SMS Allowed']);
$row = $acc_details->fetch(PDO::FETCH_NUM);
fputcsv($output, $row);
fputcsv($output, ['-----']);

fputcsv($output, ['Swimmer Details']);
fputcsv($output, ['First', 'Middle', 'Last', 'DOB', 'Squad', 'Sex', $tenant->getKey('NGB_NAME') . ' ID', $tenant->getKey('NGB_NAME') . ' Category (Internal Code)']);
...
fputcsv($output, ['-----']);
fputcsv($output, ['Medical Details']);
fputcsv($output, ['Conditions', 'Allergies', 'Medication']);
...
fputcsv($output, ['Allow Photography for']);
fputcsv($output, ['Website', 'Social Media', 'Noticeboard', 'Filming for Training', 'Professional Photographer']);
...
fputcsv($output, ['Name', 'Contact Number']);
...
fputcsv($output, ['Gala Entries (All)']);
fputcsv($output, ['Name', 'Date', '50 Free', '100 Free', '200 Free', '400 Free', '800 Free', '1500 Free', '50 Breast', '100 Breast', '200 Breast', '50 Fly', '100 Fly', '200 Fly', '50 Back', '100 Back', '200 Back', '100 IM', '200 IM', '400 IM']);
...
do {
  fputcsv($output, [$row[0], date("H:i", strtotime((string) $row[4])), date('j F Y', strtotime($row[2] . ' + ' . $row[1] . ' days')), $row[3]]);
  $row = $sessionData->fetch(PDO::FETCH_NUM);
} while ($row != null);
...
fputcsv($output, [app()->tenant->getKey('CLUB_NAME') . ' GDPR Compliance Team', 'via Membership Software', 'by SCDS']);
