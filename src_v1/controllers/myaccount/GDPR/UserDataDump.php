...
header('Content-Disposition: attachment; filename=GDpe-Data-Download-' . str_replace(' ', '-', (string) $name) . '.csv');

// create a file pointer connected to the output stream
$output = fopen('php://output', 'w');

fputcsv($output, [app()->tenant->getKey('CLUB_NAME') . ' User Data Download']);
fputcsv($output, ['Data downloaded at ' . date("H:i, d/m/Y")]);
fputcsv($output, ['-----']);

fputcsv($output, ['Account Details']);
...
fputcsv($output, ['Swimmers']);
fputcsv($output, ['First', 'Middle', 'Last', 'DOB', 'Squad', 'Sex', $tenant->getKey('NGB_NAME') . ' ID', $tenant->getKey('NGB_NAME') . ' Category (Internal ID)']);
...
fputcsv($output, ['Direct Debit Mandates']);
fputcsv($output, ['Bank Name', 'Account Holder', 'Account Num Ends', 'In Use?']);
...
fputcsv($output, ['Account Logins since ' . date("j M Y", strtotime('-120 days'))]);
fputcsv($output, ['Time', 'IP', 'IP Location', 'Browser', 'Operating System', 'Mobile']);
