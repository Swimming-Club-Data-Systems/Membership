...
<?php if (app()->tenant->getBooleanKey('PAYMENT_METHOD_INVALID') && app()->user && app()->user->hasPermission('Admin')) { ?>
    <div class="bg-danger text-light text-light-d bg-striped py-1 d-print-none">
        <div class="<?= $container_class ?>">
            <small><strong>PAYMENT METHOD MISSING OR UNUSABLE</strong> CORRECT YOUR PAYMENT INFORMATION FOR SCDS AS SOON
                AS POSSIBLE. FAILURE TO DO SO MAY RESULT IN TENANT SUSPENSION.</small>
        </div>
    </div>
<?php } ?>
    ...
    <?php if ($_SESSION['Browser']['Name'] == "Internet Explorer") { ?>
        <div class="bg-warning text-dark py-3 d-print-none">
            <div class="<?= $container_class ?>">
                ...
                <p>
                    It looks like you're using Internet Explorer which we no longer support so we recommend you upgrade
                    to a new browser which we do support as soon as possible. <strong><a class="text-dark"
                          ...
                          <img src="<?= htmlspecialchars(getUploadedAssetUrl($logos . 'logo-75.png')) ?>"
                               ...
                               style="height: 2rem">
                                ...
                                <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#chesterNavbar" aria-controls="chesterNavbar"
