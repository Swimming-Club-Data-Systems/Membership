...
<div class="container min-vh-100 mb-n3 overflow-auto">
  <!-- <h1 class="mb-5">Sign in using your <?= htmlspecialchars((string) app()->tenant->getKey('CLUB_NAME')) ?> Account</h1> -->
    ...
    <div class="col-lg-8 col-md-10">
      ...
      <div class="row align-items-center">
        <div class="col order-2 order-md-1">
          <h1 class="">Login</h1>
          <p class="">Sign in to <?= htmlspecialchars((string) app()->tenant->getKey('CLUB_NAME')) ?></p>
        </div>
      ...
      <?php if (isset($_SESSION['TENANT-' . app()->tenant->getId()]['ErrorAccountLocked']) && $_SESSION['TENANT-' . app()->tenant->getId()]['ErrorAccountLocked']) { ?>
        <div class="alert alert-danger">
          ...
          Please <a href="<?= htmlspecialchars((string) autoUrl("resetpassword")) ?>" class="alert-link">reset your password</a> in order to continue
        </div>
      <?php unset($_SESSION['TENANT-' . app()->tenant->getId()]['ErrorAccountLocked']);
      } ?>

      <form method="post" action="<?= htmlspecialchars((string) autoUrl("login")) ?>" name="loginform" id="loginform" class="needs-validation" novalidate>
        <div class="mb-3">
          <label class="form-label" for="email-address">Email address</label>
          <input type="email" name="email-address" id="email-address" class="form-control form-control-lg text-lowercase" <?php if ($errorState) { ?> value="<?= htmlspecialchars((string) $username) ?>" <?php } ?> required <?php if (!$username) { ?>autofocus<?php } ?> placeholder="yourname@example.com" autocomplete="email">
        ...
        <div class="mb-5">
          ...
          <span>
            <a href="<?= htmlspecialchars((string) autoUrl("resetpassword")) ?>" class="btn btn-dark">
              Forgot password?
            </a>
          </span>
        </div>
