      ...
      <div class="col-sm-8 col-md-5 col-lg-6 col-xl-4 align-middle">
        <div class="my-3 p-3 bg-white rounded">
          <span class="text-muted border-bottom border-gray pb-2 mb-0 d-block">
						Login with <?=htmlspecialchars(config('CLUB_NAME'))?>
					</span>
          <div class="pt-3">
						<h1 class="h4">
							<?=$app_name?> wants to access your <?=htmlspecialchars(config('CLUB_NAME'))?> Account.
						</h1>
						<p class="lead mb-0 text-truncate">
							<?=getUserName($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID'])?> <em><?=$_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['EmailAddress']?></em>
						</p>
						<hr>
						<p>
