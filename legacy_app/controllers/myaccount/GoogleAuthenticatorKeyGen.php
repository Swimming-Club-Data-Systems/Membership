...
if (!isset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey'])) {
  $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey'] = $google2fa->generateSecretKey(32);
}
...
<div class="container-xl">
  <div class="row">
    <div class="col-lg-8">
      ...
      <div class="row">
          ...
          <div class="card card-body h-100 d-grid">
            <div>
              ...
              <?php if (isset($_SESSION['Browser']['Name']) && $_SESSION['Browser']['Name'] == 'Safari' && isset($_SESSION['Browser']['Version']) && (int) $_SESSION['Browser']['Version'] >= 15) { ?>
                <p>
                  <a href="apple-<?= htmlspecialchars($google2fa->getQRCodeUrl(config('CLUB_NAME'), $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['EmailAddress'], $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey'])) ?>">Set up code with iCloud Keychain</a>.
                </p>

                <p class="mb-0">
                  <a href="<?= htmlspecialchars($google2fa->getQRCodeUrl(config('CLUB_NAME'), $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['EmailAddress'], $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey'])) ?>">Set up code with another app</a>.
                </p>
              <?php } else { ?>

                <p class="mb-0">
                  <a href="<?= htmlspecialchars($google2fa->getQRCodeUrl(config('CLUB_NAME'), $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['EmailAddress'], $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey'])) ?>">Set up time-based one-time password app</a>.
                </p>
              <?php } ?>
            </div>
          </div>
        ...
        <div class="col-md-6 pb-3">
          <div class="card card-body h-100 d-grid">
            <h3>Scan Code</h3>
            <?php
            $qr_url = urlencode($google2fa->getQRCodeUrl(config('CLUB_NAME'), $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['EmailAddress'], $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey']));
            ?>
            <div>
      ...
      <p>
        If your app does not support these setup methods, please use the code below;<br> <span class="font-monospace"><?= htmlspecialchars($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FAKey']) ?></span>
      </p>
      ...
      <?php if (isset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FA_VerifyError']) && $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FA_VerifyError']) { ?>
        <div class="alert alert-danger">
          <strong>The code you entered was not valid</strong>
        </div>
      <?php unset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['G2FA_VerifyError']);
      } ?>
