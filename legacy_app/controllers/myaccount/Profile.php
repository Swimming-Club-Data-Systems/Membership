...
$getUser->execute([$_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID']]);
...
if (!empty($_POST['forename'])) {
  if ($_POST['forename'] != $forename) {
    ...
    $update->execute([trim(mb_convert_case($_POST['forename'], MB_CASE_TITLE_SIMPLE)), $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID']]);
    $forenameUpdate = true;
    $update = true;
  }
}
if (!empty($_POST['surname'])) {
  if ($_POST['surname'] != $surname) {
    ...
    $update->execute([trim(mb_convert_case($_POST['surname'], MB_CASE_TITLE_SIMPLE)), $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID']]);
    $surnameUpdate = true;
    $update = true;
  }
}
...
if ($update) {
  ...
  $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserDetailsUpdate'] = $updateText;
}
  ...
  <div class="container-fluid">
    <div class="row justify-content-between">
      ...
      <div class="col-md-9">
        ...
        <?php if (isset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserDetailsUpdate'])) {
          $userID = $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID'];
          ...
          echo $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserDetailsUpdate'];
          unset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserDetailsUpdate']);
        } ?>
        <?php
    ...
    To complete your change of email address, please check the link in your inbox.
        ...
        <div class="">
          ...
          <div class="mb-3" id="gravitar">
            ...
            <?php
            $grav_url = "https://www.gravatar.com/avatar/" . md5(mb_strtolower(trim($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['EmailAddress']))) . "?d=" . urlencode("https://www.chesterlestreetasc.co.uk/apple-touch-icon-ipad-retina.png") . "&s=240";
            ?>
