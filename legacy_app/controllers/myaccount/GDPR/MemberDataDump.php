...
if ($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['AccessLevel'] == "Parent" && $parent != $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID']) {
  halt(404);
}

$parent_name = getUserName($parent);
$downloader_name = getUserName($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID']);
...
header('Content-Disposition: attachment; filename=GDpe-Data-Download-' . str_replace(' ', '-', $swimmer_name) . '.csv');

// create a file pointer connected to the output stream
$output = fopen('php://output', 'w');

fputcsv($output, array(config('CLUB_NAME') . ' Member Data Download'));
...
fputcsv($output, array('First', 'Middle', 'Last', 'DOB', 'Squad', 'Sex', config('NGB_NAME') . ' ID', config('NGB_NAME') . ' Category (Internal Code)'));
...
fputcsv($output, array(config('CLUB_NAME') . ' GDPR Compliance Team', 'via Membership Software', 'by SCDS'));
