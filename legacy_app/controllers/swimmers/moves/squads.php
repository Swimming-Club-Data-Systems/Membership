...
try {
  ...
  if (!Auth::User()->getLegacyUser()->hasPermission('Admin') && !Auth::User()->getLegacyUser()->hasPermission('Coach') && (isset($member) && $member->getUser()->getId() != $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['UserID'])) {
    http_response_code(404);
    echo json_encode([
      'status' => 404,
      'message' => 'Not Found'
