...
if (!isset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['AC-CC-Expected'])) {
  ...
  $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['AC-CC-Expected']['Current'] = "Parent";
  $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['AC-CC-Expected']['URL'] = "me";
  $parent = true;
  header("Location: " . autoUrl("register/ac/code-of-conduct/me"));
} else {
  if ($name != $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['AC-CC-Expected']['URL']) {
...
if ($parent) {
  ...
  $query->execute([$_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['AC-CC-Expected']['Current']]);
  $swimmer = $query->fetch();
    ...
    <div class="col-sm-10 col-md-8">
      <form method="post" class="needs-validation" action="<?=autoUrl("register/ac/code-of-conduct")?>" novalidate>
        ...
        <?php } else { ?>
          ...
          <h2><?=htmlspecialchars(config('CLUB_NAME'))?> Squad Code of Conduct</h2>
    			<?php if ($CoCID != null) { ?>
    				<div id="code_of_conduct">
        ...
        <div class="mb-3">
          <div class="form-check">
            ...
            <label class="form-check-label" for="agreement">
              I, <?=$name?> agree to the Code of Conduct of <?=htmlspecialchars(config('CLUB_NAME'))?> as
              outlined above
            </label>
