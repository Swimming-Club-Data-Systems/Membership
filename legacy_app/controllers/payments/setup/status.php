...
if (!isset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status']) || $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status'] == null) {
  halt(404);
}

$pagetitle = "You've setup a Direct Debit";
if ($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status'] != 'success') {
...
<div class="container-xl">
  <div class="row">
    <div class="col-lg-8">
      <?php if ($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status'] == 'success' || $_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status'] == 'redirect_flow_already_completed') { ?>
      ...
      <p>GoCardless handles direct debit payments for <?=htmlspecialchars(config('CLUB_NAME'))?>.</p>
      ...
      <?php } else if ($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status'] == 'redirect_flow_incomplete') { ?>
...
$_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status'] = null;
unset($_SESSION['TENANT-' . tenant()->getLegacyTenant()->getId()]['GC-Setup-Status']);